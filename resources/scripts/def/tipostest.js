...
let tblTipoTest = $("#tblTipoTest").DataTable({
    aProcessing: true,
    aServerSide: true,
    bFilter: true,
    scrollX: false,
...
const listarTiposTest=()=>{
    $('#tblTipoTest').DataTable().clear().draw();
    axios.get('listartipostest')
    .then(function (response) {
        let data=response.data
        console.log(data)
        let estado
        Array.from(data).forEach(function (tipostest) {
            if(tipostest.activo==1) {
